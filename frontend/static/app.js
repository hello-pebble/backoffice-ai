...
// items 를 넘기면 목록을 갈아끼우고, 없이 부르면 현재 목록에서 페이지만 다시 그린다.
function renderNews(items){if(items)newsItems=items;const total=newsItems.length,pages=Math.max(1,Math.ceil(total/NEWS_PER_PAGE));newsPage=Math.min(Math.max(newsPage,0),pages-1);const start=newsPage*NEWS_PER_PAGE;$('news-list').className=total?'':'empty';if(!total){$('news-list').innerHTML='아직 수집된 AI 소식이 없습니다.';return}const cards=newsItems.slice(start,start+NEWS_PER_PAGE).map(x=>`<article class="news-card"><div class="news-meta">${x.read?'':'<i class="unread-dot"></i>'}<span>${esc(x.source)}</span><span class="tag">${esc(x.category)}</span><span>${esc(x.publishedAt||x.collectedAt)}</span></div><a href="${esc(x.url)}" target="_blank" rel="noreferrer" data-news-id="${esc(x.id)}">${esc(x.title)}</a><p>${esc(x.summary)}</p></article>`).join('');const pager=pages>1?`<div class="news-pager"><button class="light" ${newsPage?'':'disabled'} onclick="newsGo(-1)">이전</button><span>${newsPage+1} / ${pages} · 전체 ${total}건</span><button class="light" ${newsPage<pages-1?'':'disabled'} onclick="newsGo(1)">다음</button></div>`:'';$('news-list').innerHTML=cards+pager}
function newsGo(step){newsPage+=step;renderNews()}
...
function renderTopicDrafts(items){const target=$('topic-draft-list');target.className=items.length?'topic-draft-list':'empty';target.innerHTML=items.length?items.map(x=>`<article class="topic-draft" id="topic-draft-${esc(x.id)}"><div class="topic-draft-head"><div><b>${esc(x.title)}</b><p class="meta"><span class="tag wait">검토 대기</span> <span class="tag ${x.slackStatus==='SENT'?'done':'late'}">${esc(SLACK_LABEL[x.slackStatus]||x.slackStatus)}</span> ${esc(x.source)} · ${esc(x.category)} · 우선순위 ${Number(x.priorityScore).toFixed(2)} · ${esc(x.createdAt.replace('T',' ').slice(0,16))}</p></div>${x.slackStatus==='SENT'?'':`<button class="light" data-notify-id="${esc(x.id)}">Slack 알림 재시도</button>`}</div><p class="hook">${esc(x.hook)}</p><pre>${esc(x.script)}</pre><p class="meta">${(x.hashtags||[]).map(t=>esc(t)).join(' ')}</p>${x.slackError?`<p class="slack-error">${esc(x.slackError)}</p>`:''}<a class="text-link" href="${esc(x.sourceUrl)}" target="_blank" rel="noreferrer">출처 원문 열기 →</a></article>`).join(''):'아직 생성된 대본 초안이 없습니다.'}
...
$('news-refresh').onclick=async()=>{const b=$('news-refresh');b.disabled=true;b.textContent='수집 중…';try{newsPage=0;renderNews(await fetch('/api/ai-news/refresh',{method:'POST'}).then(r=>r.json()))}finally{b.disabled=false;b.textContent='소식 가져오기'}};

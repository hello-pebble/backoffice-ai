...
function renderNews(items){$('news-list').className=items.length?'':'empty';$('news-list').innerHTML=items.length?items.slice(0,12).map(x=>`<article class="news-card"><div class="news-meta">${x.read?'':'<i class="unread-dot"></i>'}<span>${esc(x.source)}</span><span class="tag">${esc(x.category)}</span><span>${esc(x.publishedAt||x.collectedAt)}</span></div><a href="${esc(x.url)}" target="_blank" rel="noreferrer" onclick="newsRead('${x.id}')">${esc(x.title)}</a><p>${esc(x.summary)}</p></article>`).join(''):'아직 수집된 AI 소식이 없습니다.'}
...
async function load(){const [d,packages,news,briefing,aiOperations]=await Promise.all([fetch('/api/dashboard').then(r=>r.json()),fetch('/api/content-packages').then(r=>r.json()),fetch('/api/ai-news').then(r=>r.json()),fetch('/api/ai-news/briefing').then(async r=>r.status===204?null:r.json()),fetch('/api/ai-operations').then(r=>r.json())]);renderDashboard(d);renderContentPackages(packages);renderNews(news);renderBriefing(briefing);renderAiOperations(aiOperations)}
async function newsRead(id){await fetch(`/api/ai-news/${id}/read`,{method:'PATCH'}).then(r=>r.json()).then(renderNews)}
$('refresh').onclick=load;
$('news-refresh').onclick=async()=>{const b=$('news-refresh');b.disabled=true;b.textContent='수집 중…';try{renderNews(await fetch('/api/ai-news/refresh',{method:'POST'}).then(r=>r.json()))}finally{b.disabled=false;b.textContent='소식 가져오기'}};

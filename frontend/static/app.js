...
async function load(){const [d,packages,news,briefing,aiOperations]=await Promise.all([j('/api/dashboard'),j('/api/content-packages'),j('/api/ai-news'),j('/api/ai-news/briefing'),j('/api/ai-operations')]);if(d)renderDashboard(d);if(packages)renderContentPackages(packages);if(news)renderNews(news);renderBriefing(briefing);if(aiOperations)renderAiOperations(aiOperations)}

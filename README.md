...
| 대본 생성 | 원본(직접 입력 또는 소식·키워드에서 가져오기) 하나로 체크한 채널마다 에이전트 실행. 쇼츠 대본(검토 대기·Slack), 인스타툰 4·8컷 대본과 컷 이미지, 카드뉴스, 블로그 발행 큐. 요청은 바로 끝나고 채널은 백그라운드에서 채워짐 | Kotlin + LLM·Imagen |
...
- **워커**: `config/.env` (예시는 `config/env.example`). 스케줄 시간(`MORNING_PREP_TIME`은 아침 점검 전 소식·요약·초안 사전 준비), 콘텐츠 길이, 키워드 필터, Selenium, 재시도 설정이 여기 있습니다.

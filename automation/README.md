...
- `jobs/`: 키워드 수집(`keyword_collector.py`), 콘텐츠 생성(`content_generator.py`), 포스팅(`blog_poster.py`), 스케줄링(`scheduler.py` — 키워드·콘텐츠·포스팅 크론에 더해 `MORNING_PREP_TIME`에 백엔드 `/api/worker/morning-prep`를 불러 아침 점검용 소식·요약·초안을 미리 만듦)
